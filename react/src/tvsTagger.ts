...
import type { Polarity } from "../../shared/modeGovernance";
...
function inferDatasetPolarity(label: string): Polarity {
...
function fallbackTagFromText(
  ...
  const boundaryIrritation =
    /\bstop asking\b|\bleave me alone\b|\bdoesn't matter\b|\bwhatever\b|\bdrop it\b/.test(
      normalized
    );
  const strain =
...
function getDefaultDisplayPrefix(): string | undefined {
  const configured = import.meta.env.VITE_DISPLAY_PREFIX?.trim();
  return configured ? configured : undefined;
}

export async function tagText(userText: string, prefix?: string): Promise<TVSTagResult> {
  const displayPrefix = prefix ?? getDefaultDisplayPrefix();
  const hit = lookupSentence(userText);

  if (hit) {
    const rawTag = hit.toneSig;
    return {
      tag: renderTag(rawTag, displayPrefix),
  ...
  return {
    tag: renderTag(fallback.rawTag, displayPrefix),
